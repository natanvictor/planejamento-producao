...
Renderizado por `render_rampas_colunas(paineis, categoria_fn)` em `components/rampas_filial.py` via `st.components.v1.html`. Fundo transparente; texto adapta ao tema (`prefers-color-scheme`). Colunas **quebram para fileiras abaixo** (`flex-wrap`, sem scroll horizontal) → rampas escondidas viram continuação; `altura_paineis` estima ~6 colunas/fileira. **Ícone por linha do histórico:** 🏍️ moto verde = **na rampa agora** (id == `mid_atual` da Ativas); ✓ = finalizada (`situacao==4`); ✗ = subiu e não finalizou. **Tooltip-caixa na placa** = entrou/finalizou/nível/situação + **motivo** (último `eventoTipoDescricao`, ex.: "Retornada para fila"). Legenda com os 3 estados.
...
- **Categoria/cor** (`_categoria` em `app.py`) — **PLANO PRIMEIRO** (fix): placa no plano do dia → planejamento🟢 (seja qual for o tipo); senão, `tipo∈{1,2,5,7,10,11,12,13}`→cliente🔵, senão nao_planejamento🔴. Placa **normalizada** (sem hífen). Motivo do fix: Conquiste tem `tipoEnum=12` e Suprir Agendamento `tipoEnum=1` (faixa "cliente" do maintenance-backend), mas são **categorias do plano** → o tipo-primeiro pintava plano de azul (bug BPQ-9F62). Leitura de decisão: 🔴 = rampa gasta **fora do plano**.
- **Coluna `Ordem` na tabela da aba 1** = `ordem_prioridade` (1–7: Suprir=1, Conquiste=2, Titular=3, Limpeza c/vínc/ s/vínc=4/5, Complementares=6, Cliente=7); o df é ordenado por `Ordem, Categoria, Placa`. (NÃO confundir com a coluna `ordem` da tabela, que é rank 1..N dentro da categoria.)
- ⚠️ **Nível "—" nas finalizadas:** a API **zera o nível** ao finalizar ("Manutenção Finalizada") — some do histórico, `Detalhes/Geral` e **dos eventos**. Irrecuperável via API; existe no BQ `man_operacao.manutencoes_agrupadas.nivel_manutencao` (1–4). Fallback BQ **adiado** (decisão do Natan).
- ⚠️ **Cargo do mecânico:** `employee-management.mottu.io` recusa nosso token (issuer `sso.mottu.cloud/realms/Internal` inválido — exige `sso.mottu.io`; `realms/Admin` dá `invalid_grant` com as creds do app). **Bloqueado** até ter credencial do `sso.mottu.io`.
